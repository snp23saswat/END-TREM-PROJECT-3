...
const PRIORITY_CONFIG = {
  ...
  medium: { label: "Medium", color: "#16a34a" },
  low: { label: "Low", color: "#111111" },
};
...
function Avatar({ initials, size = 28 }) {
  const colors = ["#2563eb", "#16a34a", "#dc2626", "#111111"];
...
function Column({ column, onDragStart, onDrop, onDragOver, onDelete, onEdit, onAddCard, onRenameColumn }) {
    ...
    >
      <div className="column-header">
        ...
        <div className="column-header-actions">
          <button className="header-icon-btn" onClick={startTitleEdit} title="Rename column">✎</button>
          <button className="add-card-btn" onClick={() => onAddCard(column.id)} title="Add card">+</button>
        </div>
      </div>
...
export default function App() {
  ...
  const handleRenameColumn = (columnId, newTitle) => {
    setData((prev) => {
      const currentTitle = prev.columns[columnId].title;
      if (currentTitle === newTitle) return prev;

      alert(`Heading changed from "${currentTitle}" to "${newTitle}"`);
      return {
        ...prev,
        columns: {
          ...prev.columns,
